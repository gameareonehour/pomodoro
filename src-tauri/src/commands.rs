...
fn get_asset_path(sound_type: &str) -> Option<PathBuf> {
    ...
    if let Ok(exe_path) = env::current_exe() {
        if let Some(exe_dir) = exe_path.parent() {
            ...
            // for prod. If there is an assets folder in the same directory as the executable file.
            let prod_path = exe_dir.join("assets").join(filename);
            if prod_path.exists() {
                return Some(prod_path);
            }

            // for prod. If there is an assets folder in the parent directory of the executable file.
